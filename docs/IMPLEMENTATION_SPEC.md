...
- 空文字、異常な短さ・長さ、制御文字
- 保護用プレースホルダーの残存（桁数や区切り文字が壊れた`ZX...QXZ`形式も含む）
...
実測フォローアップでは、40件の同一コーパスについて、タイトルと概要を分離した各行の出力をJSON・CSVへ保存し、Markdownに候補別集計を出力する。実行環境、モデルリビジョン、CPU推論時間、ピークRSS、初回取得時間、キャッシュ容量を記録し、候補を実行できない場合は理由を残して計測値を補完しない。取得時間はキャッシュ増分が確認できる場合のアダプタ初期化時間として扱う。候補ごとに別プロセスで計測した場合は各プロセス内の累積ピークRSS、複数候補を同一プロセスで計測した場合は候補間で分離していない累積値であることを結果に明記する。
...
- SEO向けH1
- 試験運用バナー
- ライトテーマの共通ヘッダー（AI外電のロゴ、短い説明、最新記事・過去記事・情報源・Aboutへの導線）
- 左右2分割のヒーロー（サイト目的・価値表示と、公式RSSから原文リンクへ戻る流れを示す軽量な情報パネル）
- 最新日の個別記事抜粋と個別記事ページへのリンク
- 過去8回分へのリンク。PCは均等な2列・4行、モバイルは1列で表示する。
- 情報源一覧への導線
- 最終更新日時
...
### 20.1 Phase 2 Issue #32の公開仕様

`DESIGN_POLICY.md`を正本とし、次の共通ルールを現行サイトへ適用する。

- 色、罫線、余白、角、フォーカス表示をCSSカスタムプロパティへ集約し、白・薄灰・黒・控えめな青灰色を基調とする。不要な影、グラデーション、過剰な角丸は使わない。
- 本文と操作UIはシステムの日本語ゴシック体を使い、Webフォントを追加しない。見出し・本文・補助情報の階層と行間を確保する。
- ヘッダーはロゴと短い説明を左に置き、実在する最新記事・過去記事・情報源・Aboutの導線を右に整理する。未実装の検索やコラムを見た目だけで追加しない。
- トップのヒーローは目的・3つの価値表示と、公式RSS/Atom → AI外電 → 原文リンクの流れを示すHTML情報パネルの2分割とする。大型画像、動画背景、常時アニメーションは使わない。
- 最新記事カードは画像または既定画像と本文の2カラムを基本とし、配信元・日時・日本語見出し・原題・短報・内部記事リンク・公式リンクの区別を保つ。
- 過去ダイジェストは最新8回を均等な2列・4行で表示し、狭い画面では1列へ落とす。
- 日次、個別記事、アーカイブ、配信元一覧・詳細、About、編集・訂正ポリシー、Privacy、404は、共通のコンテナ、罫線、余白、タイポグラフィ、リンク表現を使う。
- セマンティックHTML、見出し階層、スキップリンク、視認可能なキーボードフォーカス、十分なタップ領域、`prefers-reduced-motion`を維持する。色だけで状態を示さず、装飾画像は読み上げ対象にしない。
- CSSで表現できるUIにJavaScriptを追加せず、大型依存、Webフォント、動画、追加の外部通信を導入しない。記事Markdown、翻訳、フィード、`data/seen.json`はこのデザイン改修で変更しない。
